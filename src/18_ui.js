...
.evhead button { background:#232837; border:1px solid #343b4e; color:#aab2c0; border-radius:9px;
  padding:10px 16px; font-size:15px; font-weight:700; min-width:44px; min-height:40px; }
...
.filesbtn { background:#161922; border:1px solid #2a2e3a; color:#8b93a5; border-radius:8px;
  padding:3px 9px; font-size:9.5px; font-weight:700; letter-spacing:0.08em; }
.ledgerrow { display:flex; align-items:baseline; gap:7px; padding:7px 4px; border-bottom:1px solid #1c2029;
  font-size:9.5px; }
.ledgerrow .lplate { font-family:monospace; font-weight:700; color:#ffc84a; width:64px; flex-shrink:0; }
.ledgerrow .lkind { color:#8b93a5; flex:1; min-width:0; line-height:1.35; }
.ledgerrow .ldet { font-weight:800; letter-spacing:0.04em; text-align:right; flex-shrink:0; line-height:1.35; }
.ledgersum { display:flex; gap:10px; flex-wrap:wrap; font-size:9.5px; color:#8b93a5; margin:2px 0 8px; }
.ledgersum b { color:#d8dde8; }
...
`;
  ...
  function init(uiRoot, gameHooks) {
    ...
    els.hud.innerHTML = `
      <div class="hudrow">
        ...
        <button class="filesbtn" data-key="ledger">FILES</button>
    ...
    els.hud.querySelector('[data-key=ledger]').addEventListener('click', (e) => {
      e.stopPropagation();
      if (!state) return;
  ...
  function setMatch(s) {
    ...
    closeMenu(); hideConfirm(); closeEvidenceSheet(); closeLedger();
  ...
  function update() {
    ...
    // an open ledger stays current — but only rebuilds when something
    // actually changed, so a tap can never land on a destroyed row
    if (ledgerOpen && state.time - ledgerBuiltAt > 1.0) {
      const lsig = state.cases.map(c => c.id + c.status +
        (c.status === 'ARREST' ? Math.ceil(c.convictAt - state.time) : '')).join('|');
      if (els._ledgerSig !== lsig) { els._ledgerSig = lsig; openLedger(); }
      ledgerBuiltAt = state.time;
    }
  ...
  function syncCards() {
    ...
    for (const kase of show) {
      ...
      // REBUILD ONLY WHEN CONTENT CHANGES (player-directed bug: a card
      // rewritten every frame destroys its buttons mid-tap — a thumb
      // could never land CHARGE/RELEASE). The timer bar updates in place.
      const trialS = kase.status === 'ARREST' ? Math.max(0, Math.ceil(kase.convictAt - state.time)) : '';
      const candN = kase.plate === null && kase._candPlates ? Object.keys(kase._candPlates).length : '';
      let chainSum = '';
      if (kase.status === 'OPEN' && kase.plate !== null && kase.type !== 'VANDAL' && ev.length >= 2) {
        const grS = CaseSystem.grade(state, ev, CaseSystem.gradeOpts(kase));
        if (grS.bestSum > 0 && grS.bestSum < CONFIG.Cases.CLOSURE_CONFIDENCE_SUM) chainSum = grS.bestSum;
      }
      const sig = [kase.status, kase.plate, ev.length, expSoon, trialS, candN, chainSum,
        kase.contested && kase.contested.contradiction].join('|');
      if (el._sig === sig) {
        const bar = el.querySelector('.timer i');
        if (bar) bar.style.width = (frac * 100) + '%';
        if (kase.status === 'COLD') kase._coldShownUntil = kase._coldShownUntil || state.time + 1.6;
        continue;
      }
      el._sig = sig;
  ...
  // The inferred drive between two sightings: the shortest street path
  // from the first car position to the second (player-directed — a car
  // moves along roads, so the map should too). Returns grid-space points.
  function inferLegPath(r1, r2) {
    const map = state.map;
    const s1 = map.segs[r1.segId], s2 = map.segs[r2.segId];
    const p1 = carXY(r1), p2 = carXY(r2);
    if (!s1 || !s2 || r1.segId === r2.segId) return [p1, p2];
    // route between the closest pair of segment ends
    let e1 = s1.a, e2 = s2.a, best = Infinity;
    for (const a of [s1.a, s1.b]) for (const b of [s2.a, s2.b]) {
      const d = CaseSystem.nodeDist(state, a, b);
      if (d < best) { best = d; e1 = a; e2 = b; }
    }
    const path = Traffic.route(state, e1, e2, null);
    const pts = [p1];
    if (path) for (const n of path) pts.push({ x: map.nodes[n].x, y: map.nodes[n].y });
    pts.push(p2);
    return pts;
  }

  // The lineup map: the scene, and where each lettered candidate was
  // photographed — proximity in space and time IS the deduction.
  function drawCandidateMap(cv, kase, cands) {
    const g = cv.getContext('2d');
    const map = state.map, W = cv.width, H = cv.height, m = 30;
    g.fillStyle = '#0a0c11'; g.fillRect(0, 0, W, H);
    const sn = map.nodes[kase.spawnNode];
    const pts = [{ x: sn.x, y: sn.y }, ...cands.map(c => carXY(c.read))];
    ...
    for (const s of map.segs) {
      const a = map.nodes[s.a], b = map.nodes[s.b];
      g.beginPath();
    ...
    // the scene
    g.save();
    g.translate(ox + sn.x * scale, oy + sn.y * scale);
    g.rotate(Math.PI / 4);
    g.fillStyle = '#ff5a52';
    ...
    for (const c of cands) {
      const p = carXY(c.read);
      const k = Math.round(p.x * 2) + '_' + Math.round(p.y * 2);
      const jit = (seenAt[k] || 0) * 12;
      seenAt[k] = (seenAt[k] || 0) + 1;
      const x = ox + p.x * scale + jit, y = oy + p.y * scale - jit;
      g.fillStyle = c.color;
      g.beginPath(); g.arc(x, y, 12, 0, 7); g.fill();
      g.fillStyle = '#0b0d12';
      g.fillText(c.letter, x, y + 1);
    }
    g.restore();
    g.font = '11px monospace'; g.textAlign = 'left'; g.textBaseline = 'alphabetic';
    g.fillStyle = 'rgba(10,12,17,0.8)'; g.fillRect(2, 2, 210, 36);
    g.fillStyle = '#ff5a52'; g.fillText('◆ the scene', 8, 16);
    g.fillStyle = '#c9cdd6'; g.fillText('● candidate, where it was seen', 8, 32);
  }

  // The sightings map: where each frame's CAR was, in time order, the
  // INFERRED street route between them (impossible legs stay straight
  // dashed red — a lie can't drive), and the landmarks for bearings.
  function drawCaseMap(cv, kase, ev) {
    const g = cv.getContext('2d');
    const map = state.map, W = cv.width, H = cv.height, m = 26;
    g.fillStyle = '#0a0c11'; g.fillRect(0, 0, W, H);

    // infer the drives first, so the zoom fits the whole route
    const legs = [];
    for (let i = 1; i < ev.length; i++) {
      const ok = CaseSystem.pairCoherent(state, ev[i - 1], ev[i]);
      legs.push({ ok, pts: ok ? inferLegPath(ev[i - 1], ev[i]) : [carXY(ev[i - 1]), carXY(ev[i])] });
    }
    const sn = map.nodes[kase.spawnNode];
    const pts = [{ x: sn.x, y: sn.y }, ...ev.map(r => carXY(r))];
    for (const leg of legs) pts.push(...leg.pts);
    let x0 = Infinity, x1 = -Infinity, y0 = Infinity, y1 = -Infinity;
    for (const p of pts) {
      x0 = Math.min(x0, p.x); x1 = Math.max(x1, p.x);
      y0 = Math.min(y0, p.y); y1 = Math.max(y1, p.y);
    ...
    // landmarks for bearings (muted — the route stays dominant)
    g.font = '11px monospace'; g.textAlign = 'center'; g.textBaseline = 'bottom';
    const mark = (nodeIdx, label, color) => {
      if (nodeIdx === undefined) return;
      const n = map.nodes[nodeIdx];
      if (n.x < x0 - 0.5 || n.x > x1 + 0.5 || n.y < y0 - 0.5 || n.y > y1 + 0.5) return;
    ...
    if (map.poi) {
      mark(map.poi.BANK, 'BANK', '#8a7a4a');
      mark(map.poi.GROCERY, 'GROCERY', '#4a6a52');
      mark(map.poi.OFFICE, 'OFFICES', '#4a5a78');
    }
    mark(map.syndicate, 'SYNDICATE', '#6a3a44');
    for (const e of map.exits) mark(e, 'EXIT', '#3a4152');
    // the scene
    g.save();
    ...
    // the inferred drives — impossible legs paint LAST so a clean return
    // leg on the same street can never hide them
    for (const pass of [true, false]) {
      for (const leg of legs) {
        if (leg.ok !== pass) continue;
        g.strokeStyle = leg.ok ? 'rgba(57,211,192,0.75)' : 'rgba(255,90,82,0.95)';
        g.lineWidth = leg.ok ? 3.5 : 4;
        g.setLineDash(leg.ok ? [] : [7, 6]);
        g.lineJoin = 'round';
        g.beginPath();
        leg.pts.forEach((p, i) => i === 0 ? g.moveTo(cx2(p), cy2(p)) : g.lineTo(cx2(p), cy2(p)));
        g.stroke();
        g.setLineDash([]);
      }
    }
  ...
  function openEvidenceSheet(caseId) {
    ...
    if (untied) {
      ...
      // the tie is a DEDUCTION, not a coin flip (player-directed): each
      // candidate carries where and when it was photographed relative to
      // the scene — the car seen a block away seconds after the crime is
      // the bet; letters key the cells to the map below
      const stats = {};
      for (const p of Object.keys(groups)) {
        const frames = groups[p];
        let dist = Infinity, dt = Infinity;
        for (const r of frames) {
          const s = state.map.segs[r.segId];
          if (s) dist = Math.min(dist, Math.min(
            CaseSystem.nodeDist(state, s.a, kase.spawnNode),
            CaseSystem.nodeDist(state, s.b, kase.spawnNode)));
          dt = Math.min(dt, r.t - kase.openedAt);
        }
        stats[p] = { dist, dt };
      }
      const plates = Object.keys(groups).sort((a, b) =>
        (stats[a].dist * 2 + stats[a].dt * 0.2) - (stats[b].dist * 2 + stats[b].dt * 0.2));
      const LETTERS = ['A', 'B', 'C', 'D'];
      const LCOLORS = ['#ffc84a', '#53c8ff', '#7ad9a0', '#ff9a6a'];
      if (!plates.length) {
        ...
        // where each candidate was, against the scene
        const mapWrap = h('div', 'evmap');
        const mcv = document.createElement('canvas');
        mcv.width = 720; mcv.height = 360;
        drawCandidateMap(mcv, kase, plates.map((p, i) => ({
          plate: p, letter: LETTERS[i] || '?', color: LCOLORS[i] || '#8b93a5',
          read: groups[p].slice().sort((a, b) => a.t - b.t)[0]
        })));
        ...
        plates.forEach((p, pi) => {
          ...
          const st2 = stats[p];
          const atScene = st2.dist <= CONFIG.Cases.SCENE_ID_DIST && st2.dt <= CONFIG.Cases.SCENE_ID_WINDOW;
  ...
  // ---------- the ledger: every file, its determination, and what the
  // trial revealed — a reckoning on demand (player-directed). Convictions
  // expose the truth (a wrongful one names the plate you SHOULD have
  // charged); releases never do — you don't learn what didn't happen.
  let ledgerOpen = false, ledgerBuiltAt = -9;

  function caseDetermination(kase) {
    if (kase.status === 'CLOSED') {
      if (kase.falseCharge) {
        const wrong = (kase.closedTriple || []).map(id => state.reads[id - 1])
          .find(r => r && !r.trueMatch);
        return ['WRONG PLATE' + (wrong ? ' — was ' + wrong.actualPlate : ''), '#ff8a80'];
      }
      if (kase.collapsed) return ['COLLAPSED IN COURT', '#b0813a'];
      return ['CONVICTED', '#7ad9a0'];
    }
    if (kase.status === 'ARREST') return ['TRIAL IN ' + Math.max(0, Math.ceil(kase.convictAt - state.time)) + 's', '#53c8ff'];
    if (kase.status === 'RELEASED') return ['RELEASED — NOTHING CAME OF IT', '#7ad9c0'];
    if (kase.status === 'COLD') {
      if (kase.coldReason === 'released') return ['RELEASED — THE TRAIL DIED', '#8b93a5'];
      if (kase.coldReason === 'auto-release') return ['EXPIRED UNANSWERED', '#8b93a5'];
      return ['WENT COLD', '#8b93a5'];
    }
    if (kase.status === 'LAPSED') return ['LAPSED — NO LEADS', '#5a6070'];
    if (kase.status === 'CONTESTED') return ['CONTESTED — ON YOUR DESK', '#ffb44a'];
    if (kase.plate === null) return ['COLLECTING CANDIDATES', '#b0813a'];
    return ['LIVE', '#ffc84a'];
  }

  function openLedger() {
    closeEvidenceSheet(); closeLedger(); closeMenu();
    ledgerOpen = true;
    ledgerBuiltAt = state.time;
    const sheet = h('div', 'evsheet');
    sheet.setAttribute('data-key', 'ledgersheet');
    const st = state.stats;
    const wrongful = st.falseCharges;
    const convicted = state.cases.filter(c => c.status === 'CLOSED' && !c.falseCharge && !c.collapsed).length;
    const collapsed = state.cases.filter(c => c.collapsed).length;
    sheet.innerHTML = `<div class="evhead"><b>THE FILES</b>
      <span>every case this shift ran — the determination, and what the trial showed</span>
      <button data-key="ledger-close">✕</button></div>
      <div class="ledgersum">
        <span><b>${convicted}</b> convicted</span>
        <span style="color:#ff8a80"><b>${wrongful}</b> wrongful</span>
        <span><b>${collapsed}</b> collapsed</span>
        <span><b>${st.colds}</b> cold</span>
        <span><b>${st.lapsed || 0}</b> lapsed</span>
        <span>clearance <b>${Math.floor(State.clearance(state))}%</b></span>
        <span>trust <b>${Math.floor(state.trust)}</b></span>
      </div>`;
    const live = (c) => c.status === 'OPEN' || c.status === 'CONTESTED' || c.status === 'ARREST';
    const rows = state.cases.slice().sort((a, b) =>
      (live(b) ? 1 : 0) - (live(a) ? 1 : 0) || b.openedAt - a.openedAt);
    for (const kase of rows) {
      const row = h('div', 'ledgerrow');
      row.setAttribute('data-key', 'ledger-' + kase.id);
      const [det, col] = caseDetermination(kase);
      row.innerHTML = `<span class="lplate">${kase.plate || '<i style="color:#8b93a5">no plate</i>'}</span>
        <span class="lkind">${kase.kind || kase.type} at ${kase.landmark || '—'} · ${fmtTime(kase.openedAt)}${kase.witnessDesc ? ' · "a ' + kase.witnessDesc + '"' : ''}</span>
        <span class="ldet" style="color:${col}">${det}</span>`;
      row.onclick = () => {
        if (kase.status === 'OPEN' || kase.status === 'CONTESTED') {
          closeLedger();
          openEvidenceSheet(kase.id);
        } else {
          Renderer.centerOn(kase.arrestNode !== undefined ? kase.arrestNode : kase.spawnNode);
          Renderer.pingNode(kase.arrestNode !== undefined ? kase.arrestNode : kase.spawnNode);
          identity((kase.kind || kase.type) + ' — ' + caseDetermination(kase)[0]);
        }
      };
      sheet.appendChild(row);
    }
    if (!rows.length) sheet.appendChild(h('div', 'evhint', 'No files yet. The city is warming up.'));
    sheet.querySelector('[data-key=ledger-close]').onclick = () => closeLedger();
    root.appendChild(sheet);
    els.ledger = sheet;
  }

  function closeLedger() {
    if (els.ledger) { els.ledger.remove(); els.ledger = null; }
    ledgerOpen = false;
  }
  ...
  function showVerdict(v) {
    cancelGhost(); closeMenu(); closeEvidenceSheet(); closeLedger();
    hideOverlay();
  ...
  function onTap(x, y) {
    ...
    // any sheet open? a tap on the map dismisses it (player-directed:
    // the ✕ alone was too small a door out)
    if (els.evsheet) { closeEvidenceSheet(); return; }
  ...
  return {
    ...
    openEvidenceSheet, closeEvidenceSheet, openLedger, closeLedger,
